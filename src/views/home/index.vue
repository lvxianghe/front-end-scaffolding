<template>
  <div class="home-container">
    <div class="grid-layout">
      <!-- 小型博美模块 - 替代原来的大红书位置 -->
      <div class="grid-item pomeranian" @click="navigateTo('/pomeranian')">
        <div class="content">
          ...
          <span>小型博美</span>
          <div class="description">可爱博美犬展示</div>
        </div>
      </div>
...
.home-container {
  ...
  .grid-layout {
    ...
    grid-template-rows: repeat(8, 200px); // 增加到8行
    gap: 20px;
    max-width: 1400px;
    margin: 0 auto;
    ...
    // ChatGPT模块 - 占据1x1的格子
    .chatgpt {
      grid-column: auto; // 改为自动布局，现在在grid-layout-row中
      grid-row: auto;
    ...
    // 抖音模块
    .douyin {
      grid-column: 3 / 5; // 只占第3-4列，不再占据整行
      grid-row: 7 / 8; // 在大红书上方
      background: linear-gradient(135deg, #121212, #2d2d2d); // 抖音特有的黑色渐变
    ...
    // 大红书模块 - 移至页面最底部
    .redbook {
      grid-column: 1 / 5; // 占据整行
      grid-row: 8 / 9; // 放在第8行（最底部）
      background: linear-gradient(135deg, #ff2c55, #ff385c); // 小红书特有的红色渐变
      box-shadow: 0 4px 12px rgba(255, 44, 85, 0.15);
      border: none;
      ...
      &:hover {
        transform: translateY(-5px);
        background: linear-gradient(135deg, #ff385c, #ff2c55);
        box-shadow: 0 8px 24px rgba(255, 44, 85, 0.3);
  ...
  .planner {
    ...
    grid-column: 3 / 4; // 变更为原来chatgpt的位置
    grid-row: 1 / 2;
